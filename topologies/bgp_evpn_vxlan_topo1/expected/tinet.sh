...
case "${1:-deploy}" in
  ...
  exec)
    ...
    run_worker_exec_pre
    report
    $SUDO docker exec "$(container_id "$node")" "$@" || note_failure "exec $node"
    run_worker_exec_post
    report
    ;;
  *)
