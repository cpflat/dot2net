...
container_id() {
  $SUDO docker ps -q \
    --filter "label=containerlab=host2" \
...
run_worker_deploy() {
  ...
  if $SUDO ovs-vsctl br-exists br-6a3144; then
    note_failure "bridge br-6a3144 already exists: destroy the lab that made it, or remove it by hand"
  else
    $SUDO ovs-vsctl add-br br-6a3144 || note_failure "create bridge br-6a3144"
  fi
...
run_worker_destroy() {
  ...
  $SUDO ovs-vsctl --if-exists del-br br-6a3144 || note_failure "delete bridge br-6a3144"
}
